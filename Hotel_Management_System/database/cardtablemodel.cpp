 ...
 * 向表格中插入记录
 */
bool CardTableModel::insertRecords(QString number, QString cardId, QString roomId, QDateTime checkInTime, QDateTime checkOutTime, QString isUse)
{
    ...
    QJsonObject jsonObj;
//    jsonObj["operation"] = "insert";
    jsonObj["tableName"] = "card";
    QJsonObject dataObj;
    dataObj["number"] = number;
    dataObj["cardId"] = cardId;
    dataObj["roomId"] = roomId;
    dataObj["checkInTime"] = checkInTime.toString("yyyy-MM-dd hh:mm:ss");
    dataObj["checkOutTime"] = checkOutTime.toString("yyyy-MM-dd hh:mm:ss");
    dataObj["isUse"] = isUse;
    ...
    MQTTClientHandler::instance().setTopic("website");
    MQTTClientHandler::instance().setMessage(message);

    if (MQTTClientHandler::instance().isConnected()) {
