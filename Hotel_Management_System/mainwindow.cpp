...
void MainWindow::handConnect()
{
    ...
    connect(ui->action_test,SIGNAL(triggered(bool)),this,SLOT(MqttTest()));
    ...
    connect(ui->action_Disconnect,SIGNAL(triggered(bool)),this,SLOT(SDisconnect()));
...
/**
  *@brief MainWindow::MqttTest
  * mqtt测试按钮点击事件
  */
void MainWindow::MqttTest(){
    MQTTTest* mqtttest = new MQTTTest();
    QMessageBox::information(this,tr("mqtt测试"),QString("向云端publish数据"),QMessageBox::Yes);
    mqtttest->setWindowTitle("mqtt测试");
    mqtttest->exec();
    delete mqtttest;
}
